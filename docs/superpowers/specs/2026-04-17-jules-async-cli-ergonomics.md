...
Single-repo, no fragserv deploy needed yet (jules-review isn't a running service). If/when Gemma calls it, fragserv will need:
- `git pull` in `~/repos/jules-review`
- Venv may need rebuilding if Python drift (same pattern we just hit with tts-stt)
- `.env` needs `JULES_API_KEY` + `JULES_GITHUB_USER` on fragserv
...
3. Defer fragserv deploy until Gemma integration actually starts.
